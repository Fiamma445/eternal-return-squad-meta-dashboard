...
배틀로얄 게임에서 1등만 보면 운이 많이 섞입니다. 마지막 안전지대 위치, 교전 타이밍, 제3자 개입 같은 요소 때문에 한 번의 우승만으로 조합을 좋다고 말하기 어렵습니다. 그래서 우승 여부보다 안정적으로 상위권에 들어갔는지를 보기 위해 `Top3`를 기준으로 삼았습니다. 한 번 터진 조합보다 반복해서 상위권에 들어가는 조합을 보고 싶었기 때문입니다.
...
딥러닝까지는 이어가지 않았습니다. 현재 데이터는 이동 경로, 스킬 사용 순서, 교전 로그처럼 시간 흐름이 살아 있는 데이터가 아니라 팀 단위 tabular 데이터에 가깝습니다. 그래서 복잡한 모델을 붙이기보다 LightGBM처럼 해석과 재현이 쉬운 모델로 검증하는 편이 이 프로젝트에는 더 맞다고 봤습니다.
